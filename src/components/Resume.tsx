...
import { Mail, Phone, MapPin, Github, Globe, Calendar, ExternalLink } from 'lucide-react';
...
export const Resume: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 py-12 px-4 print:p-0 print:bg-white">
      <div className="max-w-5xl mx-auto bg-white shadow-2xl rounded-3xl overflow-hidden print:shadow-none print:rounded-none flex flex-col md:flex-row">
        
        {/* Sidebar */}
        <div className="w-full md:w-80 bg-slate-900 text-white p-10 flex flex-col gap-10">
          <div className="flex flex-col gap-4">
            <h2 className="text-2xl font-bold border-b border-white/20 pb-2 uppercase tracking-widest">Profile</h2>
            <p className="text-slate-400 text-sm leading-relaxed">{personalInfo.profile}</p>
          </div>

          <div className="flex flex-col gap-4">
            <h2 className="text-2xl font-bold border-b border-white/20 pb-2 uppercase tracking-widest">Education</h2>
            <div className="space-y-6">
              {education.map((edu, i) => (
                <div key={i}>
                  <p className="text-primary font-bold text-xs mb-1">{edu.duration}</p>
                  <h3 className="font-bold text-sm mb-1">{edu.degree}</h3>
                  <p className="text-slate-400 text-xs">{edu.institution}</p>
                  <p className="text-primary text-xs mt-1 font-bold">Score: {edu.score}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            <h2 className="text-2xl font-bold border-b border-white/20 pb-2 uppercase tracking-widest">Skills</h2>
            <div className="space-y-4">
              {Object.entries(skills).map(([category, items]) => (
                <div key={category}>
                  <h3 className="text-xs font-bold text-primary uppercase mb-2 tracking-tighter">{category}</h3>
                  <div className="flex flex-wrap gap-1.5">
                    {items.map(item => (
                      <span key={item} className="bg-white/10 text-[10px] px-2 py-1 rounded-md text-slate-300 border border-white/5">
                        {item}
                      </span>
                    ))}
                  </div>
        ...
        <div className="flex-1 p-12">
          <div className="flex flex-col md:flex-row justify-between items-start mb-12 gap-6">
            <div>
              <h1 className="text-6xl font-black tracking-tighter uppercase mb-2">{personalInfo.name}</h1>
              <p className="text-primary font-bold text-xl tracking-widest uppercase">{personalInfo.title}</p>
            </div>
            <div className="flex flex-col gap-2 text-sm text-slate-500 font-medium">
              <div className="flex items-center gap-2"><Mail size={16} className="text-primary" /> {personalInfo.email}</div>
              <div className="flex items-center gap-2"><Phone size={16} className="text-primary" /> {personalInfo.phone}</div>
              <div className="flex items-center gap-2"><MapPin size={16} className="text-primary" /> {personalInfo.location}</div>
              <div className="flex items-center gap-2"><Github size={16} className="text-primary" /> {personalInfo.github}</div>
              <div className="flex items-center gap-2"><Globe size={16} className="text-primary" /> Portfolio: https://sachin-portfolio.run.app</div>
            </div>
          </div>
          ...
          <div className="space-y-12">
                  ...
                  <div key={i} className="relative pl-6 border-l-2 border-slate-100">
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <p className="text-primary font-bold text-sm uppercase tracking-widest">{exp.role}</p>
                      </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 gap-8">
                {/* Personal Projects */}
                <div className="space-y-6">
                  <h3 className="text-xs font-bold text-primary uppercase tracking-widest mb-4">Personal Projects</h3>
                  {personalProjects.map((proj, i) => (
                    <div key={i} className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                      <div className="flex justify-between items-start mb-3">
                        <h4 className="text-lg font-bold">{proj.name}</h4>
                        <a href={proj.link} target="_blank" className="text-primary"><ExternalLink size={16} /></a>
                      </div>
                      <p className="text-sm text-slate-600 mb-4">{proj.description}</p>
                      <div className="flex flex-wrap gap-2">
                        {proj.tags.map(tag => (
                          <span key={tag} className="text-[10px] font-bold uppercase bg-white px-2 py-1 rounded border border-slate-200 text-slate-500">{tag}</span>
                        ))}
                      </div>
                    </div>
                  ))}
                ...
                {/* Office Projects */}
                <div className="space-y-6">
                  <h3 className="text-xs font-bold text-primary uppercase tracking-widest mb-4">Professional Projects</h3>
                  {inOfficeProjects.map((proj, i) => (
                    <div key={i} className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="text-lg font-bold">{proj.name}</h4>
                        {proj.client && <span className="text-[10px] font-bold uppercase text-primary bg-primary/10 px-2 py-1 rounded">{proj.client}</span>}
                      </div>
                      <p className="text-xs text-slate-400 mb-3 font-bold uppercase tracking-tighter">{proj.duration}</p>
                      <p className="text-sm text-slate-600 mb-4">{proj.description}</p>
                      {proj.technologies && (
                        <div className="mb-4">
                          <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Technologies</p>
                          <p className="text-xs text-slate-500 italic">{proj.technologies}</p>
                        </div>
                      )}
                      {proj.role && (
                        <div>
                          <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Key Contributions</p>
                          <ul className="text-xs text-slate-600 space-y-1 list-disc pl-4">
                            {Array.isArray(proj.role) ? proj.role.map((r, idx) => <li key={idx}>{r}</li>) : <li>{proj.role}</li>}
                          </ul>
                        </div>
                      )}
                    </div>
          ...
          <div className="mt-12 pt-12 border-t border-slate-100 flex justify-between items-center print:hidden">
            <p className="text-slate-400 text-sm italic">Generated from professional portfolio platform.</p>
            <button 
              onClick={handlePrint}
